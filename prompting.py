...
def create_prompt(sentence, k):
    ...
    prompt.append("Translate the following request into a SQL query.")
    prompt.append("Output ONLY the SQL query.")
    prompt.append("")
    prompt.append("Database schema:")
    prompt.append(SCHEMA_TEXT)
    prompt.append("")
    
    # Add k examples to the prompt
    for i in range(k):
        prompt.append(f"Example {i+1}:")
        prompt.append(f"Request: {EXAMPLE_INPUTS[i]}")
        prompt.append(f"SQL: {EXAMPLE_OUTPUTS[i]}")
        prompt.append("")
        
    prompt.append("Now, write the SQL query for the following request.")
    prompt.append(f"Request: {sentence}")
    prompt.append("SQL:")
...
def eval_outputs(eval_x, eval_y, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    Inputs:
        * eval_x (List[str]): Evaluation natural language inputs.
        * eval_y (List[str] or None): Evaluation SQL labels if available.
        * gt_sql_path (str): Path to ground-truth SQL queries.
        * model_sql_path (str): Path to generated SQL queries.
        * gt_record_path (str): Path to ground-truth records.
        * model_record_path (str): Path to generated records.

    Returns:
        * sql_em: SQL exact match score.
        * record_em: Record exact match score.
        * record_f1: Record F1 score.
        * model_error_msgs: Error messages from executing generated SQL.
        * error_rate: Fraction of generated queries that caused SQL errors.
    '''
    ...
    error_rate = sum(msg != "" for msg in model_error_msgs) / len(model_error_msgs)
...
def main():
    ...
    # Global variables for use in prompt creation
    global SCHEMA_TEXT, EXAMPLE_INPUTS, EXAMPLE_OUTPUTS
    EXAMPLE_INPUTS = train_x
    EXAMPLE_OUTPUTS = train_y
    SCHEMA_TEXT = read_schema(os.path.join(data_folder, "flight_database.schema"))
    ...
    for eval_split in ["dev", "test"]:
        ...
        model_sql_path = os.path.join(f'results/llm_{experiment_name}_{eval_split}.sql') #for the submission, make sure to have the correct file name (see README.md)
        model_record_path = os.path.join(f'records/llm_{experiment_name}_{eval_split}.pkl') #for the submission, make sure to have the correct file name (see README.md)
